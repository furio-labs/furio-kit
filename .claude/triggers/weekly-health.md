...
3. **Open Dependabot PRs:** Run `gh pr list --author "dependabot[bot]" --state open` and list any PRs that weren't auto-merged. For each, explain why it might need manual review (likely a major bump).

4. **Security audit:** Run `pnpm audit` and report any new vulnerabilities. Compare against the most recent Weekly Health Report issue to identify what's new.

5. **Documentation sync:** Check that these files are accurate:
